...
QUnit.test("encoder-decoder", async (assert) => {
    ...
    const tests = [
        ...
        },
        {
            index: 25,
            test: (elm) => {
                assert.ok(elm.name === "Duration" && elm.type === "f" && elm.value === 87336);
            }
        },
        {
            index: 26,
            test: (elm) => {
                assert.ok(elm.name === "MuxingApp" &&
                    elm.type === "8" &&
        ...
        },
        {
            index: 28,
            test: (elm) => {
                assert.ok(elm.name === "DateUTC" &&
                    elm.type === "d" &&
                    elm.value instanceof Date);
                // toISOString
                assert.ok(elm.type === "d" &&
                    _1.tools.convertEBMLDateToJSDate(elm.value).getTime() ===
...
function create_recorder_helper_test(file) {
    return async (assert) => {
        ...
        reader.addListener("cluster", (ev) => {
            ...
            const assertion = data.every((elm) => elm.name === "Cluster" ||
                elm.name === "Timestamp" ||
