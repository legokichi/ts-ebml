...
function putRefinedMetaData(metadata, info) {
    ...
    var ebmlSize = new EBMLEncoder_1.default().encode(ebml).byteLength;
    var payloadSize = new EBMLEncoder_1.default().encode(payload).byteLength;
    var segmentTag = new exports.Buffer([0x18, 0x53, 0x80, 0x67]); // Segment
    var prevPayloadSize = payloadSize;
    ...
    var i;
    for (i = 1; i < 20; i++) {
        var segmentSize = segmentTag.byteLength + exports.writeVint(prevPayloadSize).byteLength;
        var sizeDiff = ebmlSize + segmentSize + prevPayloadSize - originalPayloadOffsetEnd;
        var segmentOffset = ebmlSize + segmentSize;
        var refined = refineMetadata(payload, sizeDiff - segmentOffset, info);
        var refinedSize = new EBMLEncoder_1.default().encode(refined).byteLength; // 一旦 seekhead を作って自身のサイズを調べる
...
function refineMetadata(mesetadata, sizeDiff, info) {
    var duration = info.duration, clusterPtrs = info.clusterPtrs, cueInfos = info.cueInfos;
