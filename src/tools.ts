...
export const Buffer: typeof global.Buffer = global.Buffer;
...
export function makeMetadataSeekable(
  ...
  let cuesSize = 5 + cuesInfo.length * 15;      // very rough initial approximation, depends a lot on file size and number
  // of CuePoints                   
...
export function putRefinedMetaData(
  ...
  const segmentSizeBuf = new Buffer('01ffffffffffffff', 'hex'); // Segmentの最後の位置は無数の Cluster 依存なので。
...
function encodedSizeOfEbml(refinedMetaData: EBML.EBMLElementBuffer[]): number {
  ...
  return refinedMetaData.reduce<ArrayBuffer[]>((lst, elm) =>
    lst.concat(encorder.encode([elm])), []).reduce((o, buf) => o + buf.byteLength, 0);
}
...
function create_seekhead(metadata: (EBML.EBMLElementDetail | EBML.EBMLElementBuffer)[], sizeDiff: number): EBML.EBMLElementBuffer[] {
  ...
  ["Info", "Tracks", "Cues"].forEach((tagName) => {
    const tagStarts = metadata.filter((elm) =>
      elm.type === "m" &&
      elm.name === tagName &&
      elm.isEnd === false
